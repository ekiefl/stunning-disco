...
class AddRaptorXStructure():
    ...
    #   define a list of all append methods in this class
        self.all_append_methods = get_append_methods()
    ...
    def add_columns(self):
        """
        This function calls all the append methods in self.methods_list
        """
    ...
    def get_append_methods():
        """
        This function defines a list of all the append methods. It searches for all the methods
        in this class that start with the string 'append_'.
        """
        append_methods = [func for func in dir(self) if callable(getattr(self, func)) and func[:7] == "append_"] 
    ...
    def append_8_class_structure(self):
        ...
        #   ss8_X are also dropped to cut down on table.saav_table_size, but code could be modified to retain them
            ss8 = ss8.drop(["AA"]+l, axis=1)
        #   merge with original dataframe
            return pd.merge(x,ss8)
        
        table.saav_table_grouped = self.table.saav_table.groupby("corresponding_gene_call")
        self.table.saav_table = table.saav_table_grouped.apply(calc_ss8_data_in_1gene)


    def append_3_class_structure(x):
        """ 
...
class VariantsTable():
    ...
    #   filter by genes and samples
        self.filter_table(self, "sample_id", self.samples)
        self.filter_table(self, "corresponding_gene_call", self.genes)
    ...
    def get_columns(self):
        ...
        """
        self.columns = self.saav_table.columns.values
    ...
    #   if there is no file name provided, return all genes in SAAV table
        if not self.genes_list_fname:
            self.genes = list(self.saav_table["corresponding_gene_call"].unique())
    #   otherwise return genes_list
        self.genes_list = [int(x.strip()) for x in open(self.out).readlines()]
    ...
    def load_samples_file_as_list(self):
        ...
        """
    #   if there is no file name provided, return all genes in SAAV table
        if not self.gene_list_fname:
            self.genes = list(self.saav_table["sample_id"].unique())
    #   otherwise return genes_list
        self.genes_list = [x.strip() for x in open(self.out).readlines()]
...
class MoleculeOperations():
    ...
    #   get sample names that have saavs in this protein
        ...
        self.genes_of_interest = np.loadtxt(self.gene_list_fname).astype(int)
