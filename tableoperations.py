...
class TableOperations():
    ...
    def __init__(self,args):
        ...
        self.file_access_dir = A("file-access-dir")
...
args = {"saav-table":"TIM_barrels",
        ...
        "columns-to-append":["solvent_accessibility","3_class_structure","8_class_structure"],
...
x = TableOperations(args)

def func(y):
    l = []
    for C in ["H","E","C","U"]:
        l.append(np.sum(y["ss3"]==C) / y["ss3_genewide_{}".format(C)][0] / 74)
    print(l)
    return l

x.genes_of_interest = [int(z) for z in x.genes_of_interest]

ss3_results = pd.DataFrame({},columns=("H_dens","E_dens","C_dens","U_dens"),index=x.genes_of_interest)
for gene in x.genes_of_interest:
    print(type(gene))
    print(x.saav_table["corresponding_gene_call"][0])
    print(x.saav_table[x.saav_table["corresponding_gene_call"]==gene])
    ss3_results.loc[gene,:] = func(x.saav_table[x.saav_table["corresponding_gene_call"]==gene])
...
#print("\n\n\n")
#function_names = [func for func in dir(GeneMetrics) if callable(getattr(GeneMetrics, func))] 
#function_names = [func for func in function_names if "__" not in func]
#
##potentially useful values for gene metrics
#extra = {"num_samples":74}
#for name in function_names:
#    function = getattr(GeneMetrics,name)
#    function(x.saav_table,extra)
